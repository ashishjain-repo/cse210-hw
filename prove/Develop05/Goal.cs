...
class Goal
{
        ...
        try
        {
            for (int i = 0; i < Goals.Count(); i++)
            {
                if (Goals[i]["Goal-Done"] == "X")
                {
                    ...
                    {
                        Points += Convert.ToInt32(Goals[i]["Goal-Bonus"]) *
                                  Convert.ToInt32(Goals[i]["Goal-Bonus-Points"]);
                    }
                }
            }
        }
    ...
    public void GoalEntry(Dictionary<int, Dictionary<string, string>> Goals, Dictionary<string, string> TempGoals,
        int Count)
    {
    ...
    public (Dictionary<int, Dictionary<string, string>>, int) LoadGoals()
    {
        ...
        Dictionary<int, Dictionary<string, string>> localDict =
            JsonSerializer.Deserialize<Dictionary<int, Dictionary<string, string>>>(jsonString);
